  ...
  \begin{@twocolumnfalse}
    \maketitle
        \begin{abstract}
            ...
            \emph{WASP-44 b is an exoplanet orbiting 
            ...
            radius (\cite{Morton}). We analyse some images obtained with Copernico 
            telescope at the Asiago Astrophysical Observatory and, after proper correction, 
            we use them to extract the light curve of the alleged planet via the TASTE project pipeline. Another 
            lightcurve is extracted from TESS observational data, which cover 
            multiple transit events, and both are duly compared. A radial velocity analysis yields further 
            insight on the mass and kinematical properties of the planet. 
            Finally, we make some considerations about limb darkening effect 
            through different methods (\cite{claret2011}, \cite{claret2017}, 
            \cite{claret2018})}.
        \end{abstract}
        \vspace*{1cm}
    \end{@twocolumnfalse}
...
\begin{figure}[h]
    \centering  
    \includegraphics[scale=0.2, angle=0]{../pictures/exo_dischist.png}
...
\end{figure}
The geometry of the system places a strong constraint on the inclination of the orbital 
plane of the planet with respect to the line of sight: the transit can be 
observed exclusively for almost edge-on orbits, meaning orbital planes that 
are as parallel as possible to the line of sight. This condition yialds a low probability of detection, which in turn requires a huge number of target stars to carry on successfull observations.
...
TASTE project 
\footnote{The Asiago Search for Transit timing variations of Exoplanets} 
(\cite{Nascimbeni}) provides us with data by Asiago Copernico 1.82m telescope 
obtained from the observations of our target. 
The dataset used only contains one transit, to make the analysis simpler.
Before actually analysing these data, we need to properly correct them: it 
is of capital importance to remove
instrumental effects in order to largely improve the final result.
After proper discussion and conversion into the required format, we will prepare
the configuration file for the analysis. 
% We'll talk about pyorbit later, so I commented " PyORBIT run". 
...
\end{itemize}
For pre-reductions steps, we use the \textit{STARSKY} subroutine \textit{HUGGY}
(\cite{Nascimbeni}). We first perform bias
...
\begin{figure}
    \centering  
    \includegraphics[scale=0.35, angle=0]{../pictures/pre-reduction/bias.png}
    \includegraphics[scale=0.35, angle=0]{../pictures/pre-reduction/master_bias.png}
    \includegraphics[scale=0.45, angle=0]{../pictures/pre-reduction/bias_comp.png}
...
\begin{figure}
    \centering  
    \includegraphics[scale=0.35, angle=0]{../pictures/pre-reduction/flat.png}
    \includegraphics[scale=0.35, angle=0]{../pictures/pre-reduction/master_flat.png}
    \includegraphics[scale=0.45, angle=0]{../pictures/pre-reduction/flat_comp.png}
...
$a_1=5.97$ and $a_2=8.76$ respectively. 

%    re-estimated x (pix):  169.85
%    re-estimated y (pix):   37.69

Photometry analysis is carried out by another subroutine, \textit{sentinel.e}, 
which requires the coordinates of the target and of the reference stars, followed by 
...
(\ref{fig:saturation-quality}). We also plot the 
...
We can can finally visualise the transit in all three cases (\ref{fig:transits}). 
To do that, we plot the ratio between target and reference star: \textit{differential 
...
selecting an ingress and egress time (\ref{fig:fit_setting}). 
A quadratic law ($p_0 x^2 + p_1 x +p_2$)
...
detection efficiency (SDE) compared to the Box-fitting Least Square (BLS) method, therefore being more accurate but
more time-consuming. 

Proof of this is the periodogram (\ref{fig:TLS_periodogram}) and folded lightcurve (\ref{fig:detrended_transits}) 
with a SDE of 15.15413, a preliminary period  $P=2.42547 d$, central transit time 
...
with literature $P=2.423804\pm0.000009 d$ (\cite{Anderson}) within 1$\sigma$.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.25, angle=0]{../pictures/tess/lctess.png}
    \caption{TESS lightcurve}
   \label{fig: lc1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.45, angle=0]{../pictures/taste/lctaste.png}
...
\clearpage
\subsection{TESS-TASTE comparison}
...
Linear ephemeris test shows that TASTE has a variation $\Delta$ with 
...
yields a variation O-C=$-0.011307d$ between observed TASTE value and predicted one.
That correspond to roughly 16.28 minutes. 
...
two results. 
%The O-C variation is in fact too little to justify the presence of any polluting signal.
The O-C variation is in fact too little to be caused by any polluting signal that could lead to a false positive detection, and therefore we are even more confident to be dealing with exoplanetary transits in our analysis.
...
analysis and Gaussian priors obtained from TESS best-fit values were set on $P$ and $Tc$ .
...
The two models yield very similar results both in terms of value and error, collected 
in tables \ref{table:3} and \ref{table:4}. This observation signals that 
the mother star $WASP-44$ is pretty inactive, as also shown from CaII observations (\cite{Turner}). The simplest model is chosen over \textit{harmonics}
...
transit time $T_c=2458386.5782\pm0.0007d$. As expected, both values are also 
compatible with TESS and TASTE estimates.
Morevoer, we obtain a radial velocity amplitude $K=137.52^{+10.55}_{-10.09}$
which is fully compatible with the literature value $K=138.8\pm9.0$. Same for 
$M_p = 0.88\pm0.07$, to be compared with $M_p = 0.89 \pm 0.06$. 
The huge errorbars found on the jitter term make it far from significative, though there is no evidence of orbital external perturbation by any gravitational source.
...
\begin{figure}[h]
	...
	\caption{Phase-folded curve from the \textit{radial velocities} model. No systematic effect is detected on the residuals, 
...
\section{Conclusions}
The main goals of this work were to extract
the lightcurve both from TASTE and TESS
data, to compute transit time of TESS (trustable
because obtained from several transits) and
TASTE and to check whether the latter falls in-
...
and eccentricity largely compatible with 0, thus 
in agreement with the tidal circularization hypothesis.
It orbits with a period of $P=2.42380\pm0.00017d$ 
around a pretty inactive parent star. For the period, 
the TESS prediction can be favoured over the value obtained from TASTE, since it comes from an exhaustive dataset.
Furthermore, no evidence of orbital perturbation was 
found, proving that no third body is perturbing 
