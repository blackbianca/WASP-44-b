...
We were then able to download the 2-min cadence target pixel file (TPF), the SAP (Simple Aperture Photometry) and 
...
Subsequently we repeat the quality selection analysis on the lightcurve files (provided by the 
TESS team and obtained from the MAST portal as previously addressed), and plot both the optimal 
aperture, the SAP and the PDCSAP fluxes thus obtained as a function of time. We can be sure of 
the correctness of the analysis we carried on from the observed overlapping between the the 
optimal aperture and SAP lightcurve. For our photometric analysis, however, we will make use of 
...
PDCSAP light curve a biweight filter with a time window of 1d and a hspline filter with a time window of 2d 
